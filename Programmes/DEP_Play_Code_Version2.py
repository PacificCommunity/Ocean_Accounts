...
# Load STAC Items
data = load(items, bbox=bbox, bands=["rededge1"], chunks={})

# Plot Mangroves
data.rededge1.plot.imshow(
    col="time",
    col_wrap=4,
    levels=[0, 1, 2, 3],
...
for time in data.time.values:
    ...
    data_year.rededge1.odc.write_cog(Graphics_Path1, overwrite = True)
