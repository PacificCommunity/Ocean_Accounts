...
##
   ...
   ##
      ...
      Regression_Set <- do.call(rbind,
                                     lapply(1:length(Sample), 
                                             function(x){if((x/1000) == trunc((x/1000))) print(x)
                                                         return(
                                                                  tryCatch(st_sf(data.frame(ESA_Value = values(crop(ESA, ext(Red[Sample[x], drop=FALSE])))[1],
                                                                                          Red_Cell_Number = Sample[x],
                                                                                          Red_Values   = Red[Sample[x]][1],
                                                                                          Green_Values = Green[Sample[x]][1],
                                                                                          Blue_Values  = Blue[Sample[x]][1]),
                                                                                          geom = st_as_sf(as.polygons(Red[Sample[x], drop=FALSE]))$geometry),
                                                                        warning = function(w) {}, 
