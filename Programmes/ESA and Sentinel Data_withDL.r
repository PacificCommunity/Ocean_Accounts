...
##
##    ok we're done with logistic regression - lets try a deep learning model
##
   load("Data_Intermediate/Test_Set.rda")
   load("Data_Intermediate/Regression_Set.rda")
   ##
   ## Reorder the data and take a sample
   ##
   Regression_Set <- Regression_Set[sample(1:nrow(Regression_Set), nrow(Regression_Set)),]
   Test_Set       <- Test_Set[sample(1:nrow(Test_Set), nrow(Test_Set)),]

   Regression_Set <- Regression_Set[1:100000,]
   Test_Set       <- Test_Set[1:100000,]
   
   Labels <- length(unique(c(levels(Test_Set$ESA_Value_Factor), levels(Regression_Set$ESA_Value_Factor))))
   
   Train_Labels <- matrix(0, nrow = nrow(Regression_Set), 
                             ncol = length(unique(c(levels(Test_Set$ESA_Value_Factor), levels(Regression_Set$ESA_Value_Factor)))))
                             
   for(i in 1:nrow(Regression_Set))
   {
      Train_Labels[i,as.numeric(Regression_Set$ESA_Value_Factor[i])] <- 1
   }
   
   Size = max(round(c(Regression_Set$Red_Values,  Test_Set$Red_Values,
                      Regression_Set$Green_Values,Test_Set$Green_Values,
                      Regression_Set$Blue_Values, Test_Set$Blue_Values)))
   
   Train_Data <- matrix(0, 
                        nrow = nrow(Regression_Set), 
                        ncol = 3 * Size)
   
   for(i in 1:nrow(Regression_Set))
   {
      Train_Data[i,((0*Size) + round(Regression_Set$Red_Values[i]))]   <- 1
      Train_Data[i,((1*Size) + round(Regression_Set$Green_Values[i]))] <- 1
      Train_Data[i,((2*Size) + round(Regression_Set$Blue_Values[i]))]   <- 1
   }

   Test_Labels <- matrix(0, nrow = nrow(Test_Set), 
                            ncol = length(unique(c(levels(Test_Set$ESA_Value_Factor), levels(Regression_Set$ESA_Value_Factor)))))
                             
   for(i in 1:nrow(Test_Set))
   {
      Test_Labels[i,as.numeric(Test_Set$ESA_Value_Factor[i])] <- 1
   }
   ...
   for(i in 1:nrow(Test_Set))
   {
      Test_Data[i,((0*Size) + round(Test_Set$Red_Values[i]))]   <- 1
      Test_Data[i,((1*Size) + round(Test_Set$Green_Values[i]))] <- 1
      Test_Data[i,((2*Size) + round(Test_Set$Blue_Values[i]))]  <- 1
   }

   ##
   ##   the softmax activation outputs a probability distribution for the 16 label catagories
   ##

      model <- keras_model_sequential() %>%
               layer_dense(units = 64, activation = "relu", input_shape = c(3 * Size)) %>%
